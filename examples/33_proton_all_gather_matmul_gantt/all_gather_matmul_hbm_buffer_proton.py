...
"""
Example-local copy of all_gather_matmul_hbm_buffer with Proton pl.scope
instrumentation. Used only by example 33; the production op in iris.ops is
not instrumented.
"""
...
def _hbm_buffer_all_gather_matmul_kernel(
    ...
    pl.enter_scope("hbm_buffer_all_gather_matmul")
    ...
    if local_pid < fetch_threshold:
        ...
        pl.enter_scope("all_gather_stages")
        ...
        pl.exit_scope("all_gather_stages")
        ...
        for k_fg in range(NUM_FLAG_GROUPS_K):
            ...
            # Note: DSL pl.scope inside a `for` body — under software pipelining
            # (this kernel uses num_stages=2) per-iteration timing may be
            # approximate. For cycle-accurate split use TTGIR override.
            with pl.scope("gemm_wait"):
    ...
    # Single exit for the top-level scope, hoisted out of both branches so
    # Proton's compile-time balance check sees one enter / one exit at the
    # same control-flow level (previously each branch had its own exit,
    # which triggered "closed without being opened" warnings).
    pl.exit_scope("hbm_buffer_all_gather_matmul")
...
def all_gather_matmul_hbm_buffer(
    ...
    trace: bool = False,
    profile: bool = False,
    profile_name: str = "hbm_buffer_all_gather_matmul",
    profile_format: str = "trace",
) -> FusedWorkspace:
    """
    ...
    if profile:
        ...
        # buffer_type="global" uses HBM-backed event buffers (much larger than
        # the per-CTA shared-memory buffer), preventing "first N events dropped
        # due to insufficient buffer size" warnings at higher WG counts.
        proton.start(
            profile_name,
            data=profile_format,
            backend="instrumentation",
            mode=Default(buffer_type="global"),
        )
